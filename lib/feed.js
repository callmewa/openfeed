...
function publishPostToFeeds(postId, userId, cb){
  ...
  getFollowers(userId, function(err, followers){
    for(var i = 0; i < followers.length; i++){
      client.rpush(util.format('feed:%s', followers[i]), postId, cb);
      console.log("pushed " + postId + " to " + followers[i] );
    }


//    followers.map(function(followerId){
//      client.rpush(util.format('feed:%s', followerId), postId, cb);
//      console.log("pushed " + postId + " to " + followerId );
//    });
  })
}
