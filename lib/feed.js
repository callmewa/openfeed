...
function likePost(postId, userId, cb){
  ...
  console.log("likePost, save to Redis for postId=" + postId);
  client.sadd(util.format(KEYS.POST_LIKES, postId), userId, cb);
  dbPromise.genQueryPromise(
      'INSERT INTO post_like ' +
        '(post_id, like_user_id, like_time) ' +
      'VALUES (?,?,?)',
      [{value: postId, hint: cql.types.dataTypes.timeuuid},
       {value: userId, hint: cql.types.dataTypes.ascii},
       {value: moment().valueOf(), hint: cql.types.dataTypes.timestamp}]
  ).fail(function (error) {
    console.trace('likePost failed, error=' + error);
  });
}
...
function addCommentAfterCheck(comment, postAuthorUserId, postCategory, cb) {
  ...
  .then(function() {
    ...
    function(err) {
      ...
      console.error("addCommentAfterCheck part 1 error ", err, err.trace)
      cb(err);
    }
  );
}
...
//likePost(cql.types.timeuuid(), 'pkdebug');
