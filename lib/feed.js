...
function addComment( comment, cb){
  if(cb === undefined) cb = redis.print;
  client.rpush(util.format('thread:%s', comment.postId), comment.commentId);
  client.hset('comments', comment.commentId, JSON.stringify(comment), cb);
}

function getThread(postId, cb){
  Q.ninvoke(client, 'lrange', util.format('thread:%s', postId), 0, -1 )
    .then(function(comments){
      return Q.ninvoke(client, 'hmget', 'comments', comments);
    })
