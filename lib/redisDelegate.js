...
function getFeed(loadFeedParams, cb){
  ...
  // count for post that's not edited or deleted or marked as no-regular.
  ...
  .then(function(feedLastInsertTime){
    ...
    // Then, Load the latest feed post Ids
  	return Q.ninvoke(
      client, 
      'zrevrangebyscore', 
    ...
    posts.forEach(function(post) {
      var feedPost = {};
      if (!isReturnAllPostFields && 
          !post.timeMsEdited && 
          post.postStatus === "Regular") { // TODO (kpan) define constants file.
        ...
        feedPost['post'] = resultPost;
      } else {
        feedPost['post'] = post;
        feedPost['timeMsInserted'] = postIdToScore[post.postId];
      }
      
      feedPostsToReturn.push(feedPost);
    });
    
    console.log("getFeed, postsToReturn=" + JSON.stringify(postsToReturn));
