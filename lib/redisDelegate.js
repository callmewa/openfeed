...
function likePost(postId, userId, cb){
  if(cb === undefined) cb = redis.print;
  console.log("likePost, postId=" + postId + ", userId=" + userId);
  var likeTime = moment().valueOf();
  client.zadd(util.format(KEYS.POST_LIKES, postId), likeTime, userId, cb);
}
...
function getPostLikes(loadLikesParams, cb){
  ...
  var isLoadMore = loadLikesParams.timeMsLikeSince > 0;
  ...
  Q.ninvoke(
      ...
      isLoadMore ? loadLikesParams.timeMsLikeSince : '+inf', 
      '-inf',
      'WITHSCORES',
      'LIMIT',
      0,
...
function getThread(loadCommentsParams, cb){
  ...
  var isLoadMore = loadCommentsParams.timeMsCreatedSince > 0;
  
  Q.ninvoke(
      client, 
      'zrevrangebyscore', 
      util.format(KEYS.THREAD, loadCommentsParams.postId),
      isLoadMore ? loadCommentsParams.timeMsCreatedSince : '+inf', 
      '-inf',
      'LIMIT',
      0,
      maxToFetch
...
function getFeed(loadFeedParams, cb){
  ...
  var isLoadMore = loadFeedParams.timeMsInsertedSince > 0;
  ...
  .then(function(feedLastInsertTime){
    ...
    // Then, Load feed post Ids
      ...
      isLoadMore ? loadFeedParams.timeMsInsertedSince : '+inf', 
      '-inf',
      'WITHSCORES',
      'LIMIT',
      0,
    ...
    for (; i < len ; i++){
      ...
      if (i % 2 === 0) {
        ...
        var getPostPromise = Q.ninvoke(client, 'hget', KEYS.POSTS, postIdOrScore);
    ...
    posts.forEach(function(post) {
      ...
      if (!isReturnAllPostFields && false && // TODO: hold off returnPartialFields feature by now.
          !jsonPost.timeMsEdited && 
          jsonPost.postStatus === "Normal") { // TODO (kpan) define constants file.
        // The post was not edited, and the status was not changed, 
        // we only need to return partial data.
        resultPost.postId = jsonPost.postId;
        resultPost.likes = jsonPost.likes;
        resultPost.comments = jsonPost.comments;
      } else {
        resultPost = jsonPost;
        resultPost.timeMsInserted = postIdToScore[jsonPost.postId];
      }
