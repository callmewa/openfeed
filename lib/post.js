...
function saveToDb(userPost) {
  ...
  return dbPromise.genQueryPromise(
    ...
    .fail(function (error) {
      console.error("point.saveToDb error " + error);
      throw error;
    });
}

/**
 * Save product post into product_list table.
 * @param userId
 * @param productId
 */
function saveToProductList(userId, productId) {
  console.log("In saveToProductList for userId=" + userId +
      ", productId=" + productId);
  var timestampMs = moment().valueOf();
  return dbPromise.genQueryPromise(
      'INSERT INTO product_list ' +
        '(user_id, post_id, time_inserted) ' +
      'VALUES (?,?,?)',
      [userId,
       {value: productId, hint: cql.types.dataTypes.timeuuid},
       {value: timestampMs, hint: cql.types.dataTypes.timestamp}]
    )
    .fail(function (error) {
      console.error("point.saveToProductList error " + error);
      throw error;
    });
}
...
exports.saveToProductList = saveToProductList;
