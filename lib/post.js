/**
 * This file contains data for post.
 */
...
var UserPost = {
  ...
  createPost: function(postId,
    ...
    obj.status = status;
    obj.timeCreated = nowTimeMs;
    obj.timeLastupdated = nowTimeMs;
...
function saveToDb(userPost) {
  console.log("In post.saveToDb, " + JSON.stringify(userPost));
...
/**
 * Save new post to category_posts table.
 * @param userProduct
 */
function saveToCategoryPosts(postCategory, postId, timeLastupdated) {
  console.log("In saveToCategoryPosts for category=" + postCategory +
      ", postId=" + postId);
  return dbPromise.genQueryPromise(
      'INSERT INTO category_posts ' +
        '(post_category, post_id, time_lastupdated) ' +
      'VALUES (?,?,?)',
      [{value: postCategory, hint: cql.types.dataTypes.ascii},
       {value: postId, hint: cql.types.dataTypes.timeuuid},
       {value: timeLastupdated, hint: cql.types.dataTypes.timestamp}]
    )
    .fail(function (error) {
      console.error("post.saveToCategoryPosts error " + error);
    });
}

/**
 * Update category posts in Redis
 * @param postCategory post category
 * @param postId post id
 * @param timeLastupdated last updated time (sorted set score)
 * @returns promise
 */
function updateCategoryPostsInRedis(postCategory, postId, timeLastupdated) {
  var key = util.format(KEYS.CATEGORY_POSTS, postCategory);
  console.log("ZADD KEYS.CATEGORY_POSTS for key=" +
      key + ", postId=" + postId);
  // Update category-based Redis sorted set for posts. Fire-and-forget.
  return Q.ninvoke(client, 'ZADD', key, timeLastupdated, postId
  ).fail(function (error) {
...
exports.saveToCategoryPosts = saveToCategoryPosts;
exports.updateCategoryPostsInRedis = updateCategoryPostsInRedis;
